defmodule AshSql.Join do
  ...
  defp add_distinct(relationship, _join_type, joined_query) do
    if !(joined_query.__ash_bindings__.in_group? ||
      ...
      calcs =
        Enum.filter(sort, fn
          {%Ash.Query.Calculation{}, _} -> true
          _ -> false
        end)
        |> Enum.map(fn {calc, direction} ->
          {calc, calc.module.expression(calc.opts, calc.context)}
        end)
      ...
      distinct =
        Enum.map(sort, fn
          ...
          {%Ash.Query.Calculation{} = calc, direction} ->
            {expr, _} =
              AshSql.Expr.dynamic_expr(
                query,
                %Ash.Query.Ref{attribute: calc},
                query.__ash_bindings__
              )
      ...
      if query.__ash_bindings__.sql_behaviour.multicolumn_distinct?() do
        from(row in query,
          distinct: ^distinct
        )
      else
